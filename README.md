...
LegalPDF Apply History shows summary-only records of guarded applies, including private report/backup paths, applied profile/email counts, and preserved local defaults. It is read-only and intentionally does not display the full import plan or source backup payload.
