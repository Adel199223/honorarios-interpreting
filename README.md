...
For future LegalPDF reintegration work, use References -> LegalPDF Integration Preview instead of the restore controls. It accepts a backup-like JSON and optional profile mappings such as `legalpdf_pj_beja = pj_gnr_beja`, then shows profile and court-email create/update/unchanged differences with `write_allowed: false`. You can also export a private Markdown/JSON preview report under `output/integration-reports/`. This preview path never changes reference files and never creates Gmail actions.
