...
LegalPDF Apply History shows summary-only records of guarded applies, including private report/backup paths, applied profile/email counts, and preserved local defaults. Its Details button loads a read-only redacted hash/status comparison for touched profiles and court-email aliases. Its Restore plan button loads a read-only, hash-only preview of what a future rollback would restore or remove from the pre-apply backup. It intentionally does not display the full import plan, source backup payload, or raw before/after reference values, and the restore plan does not write files.
