...
- For Google Photos items, use the selected-photo bridge until full OAuth Picker import is added: choose or download one image locally, paste the visible metadata/filename/date into the Google Photos metadata box, and let the normal photo recovery pipeline handle duplicate checks, OpenAI evidence, and date-conflict questions. Do not store or display raw OAuth URLs, picker URLs, media IDs, base URLs, tokens, or client secrets.
