...
In the browser app, paste the returned Gmail connector response and click `Parse Gmail IDs`, then use `Autofill from prepared payload`. This removes the easiest-to-fumble parts of recording: copying three Gmail IDs correctly and picking the right packet or individual payload path, especially after a correction or same-day packet.
