...
The reusable caller-shim starting point lives in `scripts/legalpdf_adapter_caller.py`. It exposes the safe endpoint list, prepared-review request-field helpers, stale-token helpers, contract validation, and the injected synthetic adapter sequence used by the smoke runner, so future LegalPDF integration work can build on the same draft-only boundary instead of retyping endpoint rules.
