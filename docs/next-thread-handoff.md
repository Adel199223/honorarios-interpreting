...
- LegalPDF adapter caller shim: `scripts/legalpdf_adapter_caller.py` now centralizes the safe endpoint list, reusable HTTP JSON/multipart transport, prepared-review request fields, stale-token helper, read-only/draft-only contract validation including the nested Gmail boundary and exact prepared-review binding fields, injected synthetic sequence used by the isolated adapter smoke, a guarded live-app CLI for isolated synthetic runs, and a secret-free readiness summary.
