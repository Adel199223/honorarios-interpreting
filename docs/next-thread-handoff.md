...
- LegalPDF adapter caller shim: `scripts/legalpdf_adapter_caller.py` now centralizes the safe endpoint list, prepared-review request fields, stale-token helper, read-only/draft-only contract validation, injected synthetic sequence used by the isolated adapter smoke, and a secret-free readiness summary.
