...
- In Codex, confirm the Browser/IAB Node REPL runner can import `scripts/browser_iab_smoke.mjs` and returns `status: ready` for the default review/batch path plus `answerQuestions: true` for the numbered-answer loop and `applyHistory: true` when checking LegalPDF Apply History/Detail/Restore Plan. The shell command `--browser-iab-click-through --browser-answer-questions --browser-apply-history` should fail fast with a `node_repl_cell` handoff instead of hanging.
