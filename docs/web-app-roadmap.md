...
- LegalPDF adapter caller shim in `scripts/legalpdf_adapter_caller.py`, centralizing the draft-only endpoint list, reusable HTTP JSON/multipart transport, prepared-review field binding helpers, stale-token helper, numbered-answer helpers, read-only contract validation including exact prepared-review binding fields, injected synthetic sequence, guarded isolated-runtime CLI, and secret-free readiness summary that future LegalPDF integration code should reuse.
