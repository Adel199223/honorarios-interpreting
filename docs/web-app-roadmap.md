...
- Local Diagnostics panel in the browser app, listing the safe live smoke, source-upload smoke, supporting-attachment smoke, isolated source-upload smoke, isolated supporting-attachment smoke, Browser/IAB review smoke, Browser/IAB upload smoke, and Browser/IAB attachment smoke commands without running shell commands or touching Gmail.
...
- Browser record-helper smoke (`--browser-record-helper` after a prepared replacement or packet payload) that parses fake Gmail `_create_draft` IDs and autofills the local record form without clicking record/status endpoints or Gmail.
