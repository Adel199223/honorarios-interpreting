...
- Recent Work lifecycle filters for `active`, `drafted`, `sent`, `superseded`, `trashed`, and `not_found` records, plus row-level draft verification, local `Mark manually sent` bookkeeping after the user sends a draft in Gmail, and guarded `not_found` reconciliation after a server-side Gmail re-check confirms the draft is missing.
...
- Stale browser session guard: `/api/health` is read-only and secret-free, the topbar warns when a tab is disconnected from the local server, and server-writing actions stay blocked until the app is restarted/reloaded while `Reset workspace` remains client-side.
...
- Optional local live-app smoke runner (`scripts/local_app_smoke.py`) for checking `/api/health`, the running private app's LegalPDF-style landmarks, `Suggested Next Step` surface, draft-only endpoints, secret-free Google Photos status, and public-readiness endpoint without creating PDFs or Gmail drafts by default.
- Local Diagnostics panel in the browser app, listing the safe live smoke, source-upload smoke, supporting-attachment smoke, Python runtime doctor, isolated source-upload smoke, isolated supporting-attachment smoke, read-only LegalPDF adapter readiness, isolated LegalPDF adapter contract smoke, advanced/future isolated fake-Gmail Draft API smoke, Browser/IAB review smoke, Browser/IAB upload smoke, Browser/IAB attachment smoke, Browser/IAB answers/apply smoke (`--browser-answer-questions --browser-apply-history`), Browser/IAB record helper smoke, Python browser record helper smoke, Browser/IAB profile proposal smoke, Browser/IAB Recent Work lifecycle smoke, Browser/IAB Recent Work reconciliation smoke, Browser/IAB Manual Draft Handoff stale smoke, and Browser/IAB fake Gmail API smoke commands without running shell commands or touching real Gmail.
...
- LegalPDF adapter caller shim in `scripts/legalpdf_adapter_caller.py`, centralizing the draft-only endpoint list, read-only `/api/health` plus adapter-contract readiness probing, reusable HTTP JSON/multipart transport, caller-supplied sanitized source upload input, prepared-review field binding helpers, stale-token helper, numbered-answer helpers, read-only contract validation including the nested draft-only Gmail boundary and exact prepared-review binding fields, injected synthetic sequence wrapper, guarded isolated-runtime CLI for both built-in synthetic and caller-supplied sanitized sources, and secret-free readiness/sequence summaries that future LegalPDF integration code should reuse.
...
- Optional read-only Gmail draft-existence verification: `/api/gmail/drafts/verify` calls only Gmail `users.drafts.get`, reports whether a draft ID still exists plus safe message/thread/recipient metadata, and never writes Gmail, duplicate-index records, draft logs, or reference data. Recent Work exposes the same read-only check for draft rows, keeps manual sent-status updates as local-only lifecycle bookkeeping, and uses `/api/gmail/drafts/reconcile-not-found` to re-check Gmail before marking a missing draft `not_found` locally.
...
2. Keep growing the LegalPDF caller shim from guarded caller-supplied source smoke toward a real adapter client, keeping LegalPDF read-only until the standalone workflow is stable in daily use.
