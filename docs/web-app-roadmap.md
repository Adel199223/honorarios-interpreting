...
- LegalPDF adapter caller shim in `scripts/legalpdf_adapter_caller.py`, centralizing the draft-only endpoint list, read-only `/api/health` plus adapter-contract readiness probing, reusable HTTP JSON/multipart transport, prepared-review field binding helpers, stale-token helper, numbered-answer helpers, read-only contract validation including the nested draft-only Gmail boundary and exact prepared-review binding fields, injected synthetic sequence, guarded isolated-runtime CLI, and secret-free readiness summaries that future LegalPDF integration code should reuse.
...
2. Keep growing the LegalPDF caller shim from the injected synthetic sequence toward a real adapter client, keeping LegalPDF read-only until the standalone workflow is stable in daily use.
