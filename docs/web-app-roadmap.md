...
- LegalPDF adapter caller shim in `scripts/legalpdf_adapter_caller.py`, centralizing the draft-only endpoint list, prepared-review field binding helpers, stale-token helper, numbered-answer helpers, and read-only contract validation that future LegalPDF integration code should reuse.
...
2. Grow the new LegalPDF caller shim from contract validation into a reusable full-sequence adapter client, keeping LegalPDF read-only until the standalone workflow is stable in daily use.
