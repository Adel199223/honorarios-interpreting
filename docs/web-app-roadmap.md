...
- LegalPDF adapter caller shim in `scripts/legalpdf_adapter_caller.py`, centralizing the draft-only endpoint list, prepared-review field binding helpers, stale-token helper, numbered-answer helpers, read-only contract validation, injected synthetic sequence, and secret-free readiness summary that future LegalPDF integration code should reuse.
